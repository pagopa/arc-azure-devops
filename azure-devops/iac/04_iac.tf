locals {
  ...
  base_iac_variables = {
    ...
    TF_POOL_NAME_DEV  = "arc-dev-linux-infra",
...
module "iac_code_review" {
  ...
  variables = merge(
    ...
    contains(each.value.envs, "d") && try(each.value.kv_name, "") != "" ? {
      tf_dev_aks_apiserver_url         = module.dev_secrets[each.value.name].values["arc-d-itn-dev-aks-apiserver-url"].value,
      tf_dev_aks_azure_devops_sa_cacrt = module.dev_secrets[each.value.name].values["arc-d-itn-dev-aks-azure-devops-sa-cacrt"].value,
      tf_dev_aks_azure_devops_sa_token = base64decode(module.dev_secrets[each.value.name].values["arc-d-itn-dev-aks-azure-devops-sa-token"].value),
    } : {},
    contains(each.value.envs, "u") && try(each.value.kv_name, "") != "" ? {
      tf_uat_aks_apiserver_url         = module.uat_secrets[each.value.name].values["arc-u-itn-uat-aks-apiserver-url"].value,
      tf_uat_aks_azure_devops_sa_cacrt = module.uat_secrets[each.value.name].values["arc-u-itn-uat-aks-azure-devops-sa-cacrt"].value,
      tf_uat_aks_azure_devops_sa_token = base64decode(module.uat_secrets[each.value.name].values["arc-u-itn-uat-aks-azure-devops-sa-token"].value),
    } : {},
    contains(each.value.envs, "p") && try(each.value.kv_name, "") != "" ? {
      tf_prod_aks_apiserver_url         = module.prod_secrets[each.value.name].values["arc-p-itn-prod-aks-apiserver-url"].value,
      tf_prod_aks_azure_devops_sa_cacrt = module.prod_secrets[each.value.name].values["arc-p-itn-prod-aks-azure-devops-sa-cacrt"].value,
      tf_prod_aks_azure_devops_sa_token = base64decode(module.prod_secrets[each.value.name].values["arc-p-itn-prod-aks-azure-devops-sa-token"].value),
    } : {},
